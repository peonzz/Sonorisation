...
/* Wake up from sleep period in second */
#define WAKE_UP_INTERVAL_SEC 8
/* Waking time in second */
#define WAKING_TIME_SEC 2  
...
static bool is_bluetooth_connected();
/*******************************
...
TaskHandle_t light_sleep_task_hdl;

/********************************
 * STATIC FUNCTION DEFINITIONS
 *******************************/
void configure_light_sleep() {
    esp_sleep_enable_timer_wakeup(WAKE_UP_INTERVAL_SEC * 1000000);
}
...
void light_sleep_task(void *args)
{
    while (true) {
        if (is_bluetooth_connected()) { 
            /* Délai court avant de re-vérifier */
            vTaskDelay(pdMS_TO_TICKS(1000)); 
        } else {
            /* Entrée en sommeil léger seulement si le Bluetooth n'est pas utilisé */
            esp_light_sleep_start();
            /* Réveil et attente active pour une courte période avant de vérifier à nouveau */
            vTaskDelay(pdMS_TO_TICKS(WAKING_TIME_SEC * 1000));
        }
    }
    vTaskDelete(NULL);
}
